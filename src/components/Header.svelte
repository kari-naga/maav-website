...
<nav in:fly={{ y: -100, duration: 1000 }} bind:clientHeight={height} class="fixed top-0 z-10 w-full p-4 flex gap-2 items-center font-semibold bg-slate-700">
  <NavButton href="/" active={'/' === $current.url.pathname} class="flex-shrink-0" innerClass="!p-2">
  ...
  <a href="/join" class="ml-auto">
    <Button class="flex gap-2 items-center transition-all text-white bg-blue-700 active:bg-blue-900 focus:ring focus:ring-blue-400 {'/join/' === $current.url.pathname ? '!bg-slate-800' : 'hover:bg-blue-800'}">
      <span>Join</span>
